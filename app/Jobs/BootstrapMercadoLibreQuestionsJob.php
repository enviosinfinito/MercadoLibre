...
final class BootstrapMercadoLibreQuestionsJob extends TenantAwareJob
{
    ...
    protected function handleForTenant(): void
    {
        ...
        if (! $profile->enabled) {
            Log::info('ml.questions.bootstrap.skipped_disabled', [
        ...
        try {
            ...
            foreach ($result->items as $questionPayload) {
                if (! is_array($questionPayload)) {
                    continue;
                }

                $externalId = (string) ($questionPayload['id'] ?? '');
                if ($externalId === '') {
                    continue;
                }
                ...
                $checksum = ListingFreshness::contentChecksum($questionPayload);
                ...
                ProcessCanonicalQuestionJob::dispatch(
                    $this->workspaceId,
                    $this->connectionId,
                    (int) $snapshot->id,
                );
            ...
            Log::info('ml.questions.bootstrap.page', [
